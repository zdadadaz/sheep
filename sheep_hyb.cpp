...
class RandomWalk{
    ...
    public:
        ...
        int d(){return sd;};
        int r(){return rankid;};
        int t(){return threadId;};
        void assign(int x, int y, int d){sx = x; sy=y; sd= d; rankid = (x / range_pp)%numRank; threadId = y%numThread;};
};
RandomWalk::RandomWalk(int inx, int iny, int ind){sx = inx; sy=iny; sd = ind; rankid = (inx / range_pp)%numRank;threadId = iny%numThread;}
...
void init_sheep_wolve(vector<Animal> &sheeps, int flag)
{
    ...
    #pragma omp parallel
    {
        vector<Animal> local_sheeps;
        int local_sheepNum = 0;
        int loc_initnumber = initnumber/numThread;
        #pragma omp task
        {
            // int ID = omp_get_thread_num();
            // printf("loc_initnumber %d\n", loc_initnumber);
            while (local_sheepNum < loc_initnumber)
            {
                ...
                local_sheeps.push_back(animal);
                local_sheepNum++;
            }
            #pragma omp critical
            {
                (*tmpTotl) += local_sheepNum;
                sheeps.insert(sheeps.end(), local_sheeps.begin(), local_sheeps.end());
            }
        }
    }
...
void animal_eat(vector<Animal> &sheeplist, vector<Animal> &wolflist, vector<Grassclass> &grasslist){
    ...
    vector<vector<int>> wolf2d(numThread, vector<int>(0,0));
    #pragma omp parallel
    {
        ...
        #pragma omp for
        for (int i= 0; i < numThread; i++){
            int local_size = sheep2d[i].size();
            for (int j = 0; j < local_size; j++){
                int idx = sheep2d[i][j];
                if ((Grass == 1) && (sheeplist[idx].gEnergy()>merror))
                {
                    eatGrass(sheeplist[idx], grasslist);
                }
            }
        }
        // record index for wolf
        #pragma omp for
        for (int i= 0; i < numThread; i++){
            for (int j = 0; j < vec_size_w; j++){
                 if (wolflist[j].t() == i){
                    wolf2d[i].push_back(j);
                }
            }
        }
        // wolf eat
        #pragma omp for
        for (int i= 0; i < numThread; i++){
            int local_size = wolf2d[i].size();
            for (int j = 0; j < local_size; j++){
                int idx = wolf2d[i][j];
                if (wolflist[idx].gEnergy()>merror)
                {
                    eatSheep(wolflist[idx], sheeplist);
                }
            }
        }
    }
...
int main(int argc, char** argv)
{
    ...
    // printf("rank %d, init s %d, w %d, g %d, range %d, r_st %d\n", rankId, initSheepNum_pp, initWolveNum_pp, initGrass_pp, range_pp, range_st);
    ...
    // printf("rank %d, s %d, w %d, g %d\n",rankId, tot_sheep, tot_wolve, tot_grass);
