...
void death(vector<Animal> &animals, Animal &animal)
{
...
// #pragma omp critical
    ...
    for (i = 0; i < vec_size; i++)
    {
        ...
        {
            reproduce(sheeplist, sheeplist[i]);
        }
        death(sheeplist, sheeplist[i]);
    }
}
...
void ask_wolf(vector<Animal> &wolflist, vector<Animal> &sheeplist)
{
    ...
    for (i = 0; i < vec_size; i++)
    {
        ...
        {
            reproduce(wolflist, wolflist[i]);
        }
        death(wolflist, wolflist[i]);
    }
}
